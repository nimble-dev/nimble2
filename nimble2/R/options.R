...
# sessionSpecificDll should not be needed in nimble2
# nimbleUserNamespace <- as.environment(list(sessionSpecificDll = NULL))
nimbleUserNamespace <- as.environment(list()) # style that is used in nimble instead of new.env() due to weird errors at some point
...
nimOptimMethod(
  ...
  function(par, fn, gr, he, lower, upper, control, hessian) {
    ...
    result <- nlminb(par,
      objective = fn, gradient = gr, hessian = he,
...
.nimbleOptions <- as.environment(
  list(
    # All options from nimble are commented out in nimble2 until determined to be still wanted
    ...
    checkDerivsArgs = TRUE, # Appears early in nimbleFunction()
