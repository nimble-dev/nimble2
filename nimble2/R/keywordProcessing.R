...
besselK_keywordInfo <- keywordInfoClass(
  ...
  processor = function(code, nfProc, RCfunProc) {
    ...
    if (is.numeric(expon.scaledArg) || is.logical(expon.scaledArg)) {
      ...
      code$expon.scaled <- substitute(1 + A, list(A = expon.scaledArg))
    }
    return(code)
  }
)
...
calculate_keywordInfo <- keywordInfoClass(
  ...
  processor = function(code, nfProc, RCfunProc) {
    ...
    nodeFunVec_ArgList <- list(
      model = code$model, nodes = code$nodes, wrtNodes = code$wrt,
    ...
    if (hasBracket(nodeFunVec_ArgList$nodes)) { ## like calculate(model, nodes[i]), which could have started as model$calculate(nodes[i])
      if (buildDerivs) {
        stop(paste0(
          "Derivatives for an indexed node are not allowed.\n",
    ...
    if (!useNodeFunctionVectorByIndex) {
      ...
      newRunCode <- substitute(
        ...
        list(
          NODEFUNVEC_NAME = as.name(nodeFunName),
...
calculateDiff_keywordInfo <- keywordInfoClass(
  ...
  processor = function(code, nfProc, RCfunProc) {
    ...
    # To-do: Support model$calculateDiff()
    if (isCodeArgBlank(code, "nodes")) {
      LHSnodes_ArgList <- list(model = code$model)
      LHSnodes_name <- allLHSNodes_SetupTemplate$makeName(LHSnodes_ArgList)
      addNecessarySetupCode(LHSnodes_name, LHSnodes_ArgList, allLHSNodes_SetupTemplate, nfProc, allowToCpp = FALSE)
    ...
    # To-do: Support model$calculateDiff(nodes[i])
    useNodeFunctionVectorByIndex <- FALSE
    if (hasBracket(nodeFunVec_ArgList$nodes)) { ## like calculate(model, nodes[i]), which could have started as model$calculate(nodes[i])
      useNodeFunctionVectorByIndex <- TRUE
      if (length(nodeFunVec_ArgList$nodes) != 3) stop(paste0("Problem with ", deparse(code), ". If you need to index on the nodes argument there should be only one index."))
    ...
    if (!useNodeFunctionVectorByIndex) {
      newRunCode <- substitute(
        model_calculateDiff(MODEL, NODEFUNVEC_NAME),
        list(MODEL = code$model, NODEFUNVEC_NAME = as.name(nodeFunName))
      )
    } else {
      newRunCode <- substitute(
        calculateDiff(nodeFxnVector = NODEFUNVEC_NAME, nodeFunctionIndex = NODEFUNVECINDEX),
...
simulate_keywordInfo <- keywordInfoClass(
  ...
  processor = function(code, nfProc, RCfunProc) {
    ...
    # To-do: support model$simulate(nodes[i])
    useNodeFunctionVectorByIndex <- FALSE
    if (hasBracket(nodeFunVec_ArgList$nodes)) { ## like calculate(model, nodes[i]), which could have started as model$calculate(nodes[i])
      useNodeFunctionVectorByIndex <- TRUE
      if (length(nodeFunVec_ArgList$nodes) != 3) stop(paste0("Problem with ", deparse(code), ". If you need to index on the nodes argument there should be only one index."))
    ...
    if (!useNodeFunctionVectorByIndex) {
      newRunCode <- substitute(
        model_simulate(MODEL, NODEFUNVEC_NAME),
        list(MODEL = code$model, NODEFUNVEC_NAME = as.name(nodeFunName))
      )
    } else {
      newRunCode <- substitute(
        simulate(nodeFxnVector = NODEFUNVEC_NAME, nodeFunctionIndex = NODEFUNVECINDEX),
        list(NODEFUNVEC_NAME = as.name(nodeFunName), NODEFUNVECINDEX = nodesIndexExpr)
      )
    }
    return(newRunCode)
...
getLogProb_keywordInfo <- keywordInfoClass(
  ...
  processor = function(code, nfProc, RCfunProc) {
    ...
    # To-do: Support model$getLogProb()
    if (isCodeArgBlank(code, "nodes")) {
      LHSnodes_ArgList <- list(model = code$model)
      LHSnodes_name <- allLHSNodes_SetupTemplate$makeName(LHSnodes_ArgList)
      addNecessarySetupCode(LHSnodes_name, LHSnodes_ArgList, allLHSNodes_SetupTemplate, nfProc, allowToCpp = FALSE)
    ...
    # To-do: Support model$getLogProb(nodes[i])
    useNodeFunctionVectorByIndex <- FALSE
    if (hasBracket(nodeFunVec_ArgList$nodes)) { ## like calculate(model, nodes[i]), which could have started as model$calculate(nodes[i])
      useNodeFunctionVectorByIndex <- TRUE
      if (length(nodeFunVec_ArgList$nodes) != 3) stop(paste0("Problem with ", deparse(code), ". If you need to index on the nodes argument there should be only one index."))
    ...
    if (!useNodeFunctionVectorByIndex) {
      newRunCode <- substitute(
        model_getLogProb(MODEL, NODEFUNVEC_NAME),
        list(MODEL = code$model, NODEFUNVEC_NAME = as.name(nodeFunName))
      )
    } else {
      newRunCode <- substitute(
        getLogProb(nodeFxnVector = NODEFUNVEC_NAME, nodeFunctionIndex = NODEFUNVECINDEX),
        list(NODEFUNVEC_NAME = as.name(nodeFunName), NODEFUNVECINDEX = nodesIndexExpr)
      )
    }
    return(newRunCode)
  ...
  processor = function(code, nfProc, RCfunProc) {
    ...
    if (to_ArgList$class %in% modelValuesTypes) {
      if (isCodeArgBlank(code, "rowTo")) {
        if (isCodeArgBlank(code, "row")) {
          stop("row argument missing in copy call")
        } else {
    ...
    if (!is.null(nfProc$origMethods[[deparse(fxnCall)]])) {
      ...
      for (nameToCheck in c("outInds")) { ## Oops, general loop but only one case.
        if (!non_NA_arg(code[[nameToCheck]])) {
          code[[nameToCheck]] <- substitute(A, list(A = -1))
        } # get -1 as a number, not parse tree of -(1)
      }
      for (nameToCheck in c("inDir", "outDir")) {
        if (!non_NA_arg(code[[nameToCheck]])) {
          code[[nameToCheck]] <- as.numeric(NA)
        } # ensure it is a numeric NA.
      }
...
matchFunctions[["getLogProb"]] <- function(model, nodes, nodeFxnVector, nodeFunctionIndex) {}
...
getSymObj_recurse <- function(code, symTab, recurse = FALSE) { # code will be like a$b$c with expectation all are NF or NL
  if (length(code) > 1) {
    if (deparse(code[[1]]) != "$") {
      return(NULL)
    } ## This is valid if we have makeNewNimbleListObject(...)$member or foo()$member
    firstArg <- code[[2]]
  ...
  if (recurse) {
    ...
    if (is.null(symObject)) {
      return(NULL)
    } ## will assume to be nimbleList, only kind of nested data structure that could be of unknown type right now
  } else {
    return(symObject)
  }
...
symTypeFromSymTab <- function(codeName, symTab, options = character(0)) {
  ...
  if (!(class %in% options)) {
    return(NULL)
  } ## nimbleList that was not constructed in setup code
  return(class)
}
      ...
      if (deparse(leftSide[[1]]) == "$") {
        ...
        if (length(leftLeftSide) != 1) {
          if (deparse(leftLeftSide[[1]]) == "$") { ## a$b[[i]]$foo()
            ...
            stop("Problem processing something like a$b[[i]]$foo()")
          }
        } else {
          nfListName <- deparse(leftSide[[2]])
          if (nfProc$setupSymTab$symbolExists(nfListName)) { ## look in symbol table
            symObj <- nfProc$setupSymTab$getSymbol(nfListName)
          }
        }
      }
    } else {
      symTab <- nfProc$setupSymTab
      symObj <- symTab$getSymbol(deparse(leftSide))
    }
  } else {
    symTab <- symObj <- NULL
  }
...
matchKeywords_recurse <- function(code, nfProc = NULL) {
  ...
  if (cl == 1) { ## There are no arguments
    if (is.call(code)) {
      if (length(code[[1]]) > 1) {
        if (deparse(code[[1]][[1]]) == "$") {
          code <- matchKeywordCodeMemberFun(code, nfProc)
        } else {
          code[[1]] <- matchKeywords_recurse(code[[1]], nfProc)
        } ## recurse on the "a$b" part of a$b() (or the "a(b)" part of a(b)()), etc
      } else {
  ...
  if (is.call(code)) {
    if (length(code[[1]]) > 1) {
      if (deparse(code[[1]][[1]]) == "$") {
        code <- matchKeywordCodeMemberFun(code, nfProc)
      } ## handle a$b(c) as one unit
      else {
        code[[1]] <- matchKeywords_recurse(code[[1]], nfProc)
      } ## handle "a(b)" part of a(b)(c), which is probably *never* triggered
    }
...
makeMapAccessExpr <- function(newName, newNameExpr, nDim) { ## newNameExpr not used any more!
  ...
  ans <- substitute(
    map(name, nDim, Moffset, sizes, strides),
...
varAndIndices2flatIndex <- function(varAndIndices, varInfo) {
  if (length(varInfo$maxs) == 0) {
    return(1)
  } ## A -1 is done automatically, later, so here we should stay in R's 1-based indexing
  sizes <- varInfo$maxs
  strides <- c(1, cumprod(sizes[-length(sizes)]))
...
makeMapSetupCodeNames <- function(baseName) {
  list(
    Mstrides = paste0(baseName, "_strides"),
...
nimDerivsInfoClass_init_impl <- function(
  .self,
  wrtNodes,
  calcNodes,
  model
  ...
  wrtNodeComps <- model$expandNodeNames(wrtNodes, returnScalarComponents = TRUE)
  ...
  # See comment in makeModelDerivsInfo for discussion of nodes versus components.
...
makeOutputNodes <- function(model,
  ...
  calcNodes <- model$expandNodeNames(calcNodes) # Ensure includes all encompassing nodes.
  ...
  modelOutputNodes <- c(
    model$expandNodeNames(calcNodes[isDetermCalcNodes], returnScalarComponents = TRUE),
...
getImmediateParentNodes <- function(nodes, model) {
  ...
  ## See Issue 1151. For "unusual" kinds of splitting, we can encounter a name with "%.s%"
  ## which means we need to extract the disjoint set of element names it includes
  boolSplit <- grepl("\\%\\.s\\%", fromNodes)
  if (any(boolSplit)) {
    iSplit <- which(boolSplit)
    graphIDsplit <- fromIDs[iSplit]
    fixedSplitNodes <- unlist(lapply(graphIDsplit, function(i) maps$elementNames[which(maps$elementID_2_vertexID == i)]))
    fromNodes <- unique(c(fromNodes[!boolSplit], fixedSplitNodes))
  }
  fromNodes
}
...
makeModelDerivsInfo_impl <- function(model,
  ...
  )] # Run `isStoch` on nodes not components (issue #1431).
  ...
  list(
    updateNodes = extraInputNodeComps,
  ...
  methods = list(
    initialize = function(wrtNodes = NULL,
      ...
      if (!isTRUE(thisModel$modelDef$buildDerivs)) {
        stop(paste0(
          "To use automatic differentiation involving model calculations,\n",
          "include 'buildDerivs = TRUE' in the call to 'nimbleModel' or set\n",
          "'nimbleOptions(buildModelDerivs = TRUE)' before calling 'nimbleModel'."
        ), call. = FALSE)
      }
      switch(case,
        nimDerivsCalculate = nimDerivsInfoClass_init_impl(
          .self = .self,
          wrtNodes = wrtNodes,
          calcNodes = calcNodes,
          model = thisModel
        ),
        updateOnly = nimDerivsInfoClass_update_init_impl(
          .self = .self,
          updateNodes = updateNodes,
          constantNodes = constantNodes,
          model = thisModel
        ),
        outputOnly = nimDerivsInfoClass_output_init_impl(
          .self = .self,
          calcNodes = calcNodes,
          model = thisModel
        )
