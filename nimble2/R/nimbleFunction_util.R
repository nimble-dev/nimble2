...
is.rcf <- function(x, inputIsName = FALSE, where = -1) {
  ...
  # nfMethodRC is being removed from nimble2
...
nf_getRefClassObject <- function(f) {
  ...
  if (inherits(f, "CnimbleFunctionBase")) {
    return(f)
  }
...
nf_getGeneratorFunction <- function(f) {
  ...
  if (is.nf(f)) {
    return(nf_getRefClassObject(f)$.generatorFunction)
  }
  if (is.Cnf(f)) {
    return(nf_getGeneratorFunction(f$Robject))
  }
...
nf_getInstances <- function(f) {
  if (is.nfGenerator(f)) {
    return(getFunctionEnvVar(f, "instances"))
  }
  if (is.nf(f)) {
    return(getFunctionEnvVar(nf_getGeneratorFunction(f), "instances"))
  }
  stop("invalid nimbleFunction argument\n")
}
...
nf_getSetupOutputNames <- function(f, hidden = FALSE) {
  ...
  if (is.nfGenerator(f)) {
    return(nameFunction(names(getFunctionEnvVar(f, "nfRefClass")$fields())))
  }
  if (is.nf(f)) {
    return(nameFunction(names(getFunctionEnvVar(nf_getGeneratorFunction(f), "nfRefClass")$fields())))
  }
  stop("invalid nimbleFunction argument\n")
}
